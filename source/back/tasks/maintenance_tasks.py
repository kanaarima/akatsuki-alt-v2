...
class CheckNewRankedBeatmaps(Task):
    ...
    def can_run(self) -> bool:
        return (
            datetime.now()
            - datetime.fromtimestamp(database.get_task("checknewrankedbeatmaps"))
        ) > timedelta(days=1)

    def run(self) -> TaskStatus:
        iterate_all = database.get_task("checknewrankedbeatmaps") == 0
        ...
        while True:
            ...
            if not sets:
                break
        database.set_task("checknewrankedbeatmaps", time.time())
        return self._finish()
